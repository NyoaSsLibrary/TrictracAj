...
local antiKickEnabled = true -- Safe Mode: blocks Auto L/R + Aimbot during duel countdown or while holding brainrot
local autoLeftSetVisual,autoRightSetVisual = nil,nil
local speedLabel = nil
local autoBatEnabled = false
local autoSwingEnabled = false
local autoBatSetVisual = nil
local aimbotSpeed = 59
...
local function getIntroSongName()
...
local function _safeNotify(msg)
...
local function preloadIntroSongs()
...
local function makeIntroSoundFromId(soundId, name, parent)
...
local function createIntroSound(option,fileName,parent,allowDownload)
...
local PlayerESP = {enabled = false, playerData = {}, conns = {}, discordText = "discord.gg/aceduels"}
local DEFAULT_SIDEBAR_ART_ID = "111278026666543"
local THEME_ACCENT = Color3.fromRGB(230, 230, 230)
local THEME_ACCENT_DIM = Color3.fromRGB(145, 145, 145)
...
local CURRENT_ART_ID = (V.sidebarArt ~= "" and V.sidebarArt) or DEFAULT_SIDEBAR_ART_ID
local startPlayerESP, stopPlayerESP
local unwalkSavedAnimate = nil
local _anyKeyListening = false
local _keyListenGeneration = 0
...
local antiDesyncPanelLaunchBtn = nil
...
local instaResetPanelLaunchBtn = nil
local startAntiDesyncAutoBat, stopAntiDesyncAutoBat
...
-- Semi Instant Steal: Candy Hub timing with the Ace Duels layout.
-- It primes the hold from farther away, fills to 100% over 1.3s, then waits
-- until the player enters the selected close radius before firing the steal.
do
...
local mobileHoldJumpPressed = false
local mobileHoldJumpActive = false
...
-- ============================================================
local function _akCountdownNumber(text)
	local n = tonumber(text)
	return n ~= nil and n >= 1 and n <= 5
end

local function _akGetCountdownLabel()
...
local function _akIsCarryableTool(tool)
...
local function _akHoldingBrainrot()
...
local function _akTryStart(featureName)
	if _akIsLocked() then
...
local guiSizeValue = 0.8
local mainGuiScale = nil
local setGuiSizeVisual = nil
local mobileButtonSizeValue = 1
local setMobileButtonSizeVisual = nil
local mobileButtonRefs = {}
local mobileButtonPositions = {}
local resetMobileButtons = nil
local MOBILE_BUTTON_DEFAULT_POSITIONS = {
...
local function applyMobileButtonSize()
...
local function packMobileButtonPositions()
	local packed={}
...
local setUnwalkVisual,setAntiLagVisual,setAutoSwingVisual,setSafeModeVisual
...
local function loadConfigState()
	...
	task.spawn(function()
		...
		if cfg.antiDesyncAutoBat then startAntiDesyncAutoBat() end
